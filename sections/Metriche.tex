...
\end{itemize}
Tendenzialmente i fold vengono scelti prima di effetturare la cross validation, in modo che la distribuzione delle classi in ogni fold sia simile a quella del dataset originale (\textbf{stratificazione}).
...
\end{figure} \noindent
\section{Affidabilit\`a delle misure di prestazioni}
Possiamo valutare l'affidabilit\`a delle misure di prestazioni calcolando gli intervalli di confidenza.
Supponiamo di aver avuto $S=750$ successi su $N=1000$ prove.
Il valore stimato della probabilit\`a di successo \`e $\hat{p}=\frac{S}{N}=0.75$.
Quanto \`e vicino il vero success rate $p$ a questa stima?
Con un intervallo di confidenza al $80\%$ abbiamo $p \in [73.2, 76.7]$.
